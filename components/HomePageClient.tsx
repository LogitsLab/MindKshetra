...
export type FeaturedVerse = {
  ...
  english: string;
  hindi: string;
  /** Today's Moon nakshatra when the pick was nakshatra-driven. */
...
export default function HomePageClient({ featured, previewMoods }: Props) {
  ...
  // Home as a router, not a brochure (WS3): a signed-in visitor who has not
  // practiced yet is led to today's sādhana, and the daily anchor (verse of
  // the day) renders above the catalog of path cards. Guests keep the
  // catalog-first order — exploration is the right first ask.
  const sadhanaCtaPrimary = Boolean(user) && sadhanaDone === false;
  const votdFirst = Boolean(user);
  ...
  const votdSection = (
    <section className="border-t border-[var(--hairline)] py-14">
      ...
      <div className="glass relative overflow-hidden px-6 py-10 sm:px-10 sm:py-12">
        ...
        {featured.nakshatra ? (
          <p className="mt-2 text-sm font-light text-[var(--text-soft)]">
            {t("homeVotdNakshatra").replace("{nakshatra}", featured.nakshatra)}
          </p>
        ) : null}
  ...
  return (
    <div className="relative">
      ...
      <section className="relative flex min-h-[calc(100dvh-8rem)] flex-col justify-center py-8 sm:py-16">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <div className="animate-rise-delay-3 mt-8 flex flex-wrap items-center gap-3 sm:mt-10">
            ...
            {sadhanaCtaPrimary && continueSlokaId ? (
              <Link
                href={`/sloka/${continueSlokaId}`}
      ...
      {/* WS3 ordering: returning users get the daily anchor (VOTD, then
          today's practice) before the catalog; guests keep catalog-first. */}
      {votdFirst ? (
        <>
          {votdSection}
          {practiceSection}
