...
export async function GET(request: NextRequest) {
  ...
  if (q?.trim()) {
    const results = await searchSlokas(q, 40);
    if (results.length > 0) {
      return NextResponse.json(
        { results },
    ...
    const nearest =
      didYouMean.length > 0
        ? await searchSlokas(didYouMean[0], 6)
        : await suggestNearestSlokas(q, 6);

    return NextResponse.json(
      { results: [], nearest, didYouMean },
      { headers: { "Cache-Control": "public, max-age=60, stale-while-revalidate=300" } }
    );
  }
  ...
  if (chapterParam) {
    ...
    return NextResponse.json(await getSlokasByChapter(chapter), {
      headers: { "Cache-Control": CACHE_STATIC },
    });
  }

  return NextResponse.json(await getAllSlokas(), {
    headers: { "Cache-Control": CACHE_STATIC },
  });
}
